...
function Popup(props) {
  ...
  const [dialogTitle,setDialogTitle]= useState('Join as a Creative Agency or Model?')
  ...
  return (
    <Dialog open={openPopup} onClose={handleClose} PaperProps={{sx:{padding:"40px"}}} >
      <DialogTitle>
      ...
      <DialogContent className=''>
        {!showSignUpForm ? (
          <div className='flex'>
            <button className='w-48 bg-white rounded-lg shadow p-6 hover:cursor-pointer mb-8 hover:bg-yellow-500 mr-5'
            onClick={() => {
            ...
            <button className='w-48 bg-white rounded-lg shadow p-6 hover:cursor-pointer mb-8 hover:bg-yellow-500'
            onClick={() => {
        ...
        {!showSignUpForm && selectedOption && (
          <div className='flex justify-center'>
            <RegistrationButton label={`Register as ${selectedOption}`} onClick={handleRegistrationClick} />
          </div>
        )}
      </DialogContent>
    </Dialog>
